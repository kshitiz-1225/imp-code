...
class KMeansRefine(Protonet):
    ...
    def forward(self, sample, super_classes=False):
        batch = self._process_batch(
            sample, super_classes=super_classes)  # _alphabet
        ...
        radii = Variable(torch.ones(bsize, nClusters)).cuda() * \
            torch.exp(self.log_sigma_l)
        ...
        if batch.x_unlabel is not None:
            ...
            for ii in range(self.config.num_cluster_steps):
                ...
                prob_unlabel_nograd = Variable(
                    prob_unlabel.data, requires_grad=False).cuda()
        ...
        acc_val = torch.eq(
            y_pred.squeeze(), batch.y_test.squeeze()).float().mean().data[0]
