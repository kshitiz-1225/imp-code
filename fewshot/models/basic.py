...
class Protonet(nn.Module):
    ...
    def _process_batch(self, batch, super_classes=False):
        ...
        if batch.x_unlabel is not None and batch.x_unlabel.size > 0:
            ...
            y_unlabel = Variable(torch.from_numpy(
                batch.y_unlabel.astype(np.int64)), requires_grad=False).cuda()
        else:
            x_unlabel = None
            y_unlabel = None

        if super_classes:
            labels_train = Variable(torch.from_numpy(batch.y_train_str[:, 1]).type(
                torch.LongTensor), requires_grad=False).unsqueeze(0).cuda()
            labels_test = Variable(torch.from_numpy(batch.y_test_str[:, 1]).type(
                torch.LongTensor), requires_grad=False).unsqueeze(0).cuda()
        else:
            labels_train = Variable(torch.from_numpy(batch.y_train.astype(
                np.int64)[:, :, 1]), requires_grad=False).cuda()
            labels_test = Variable(torch.from_numpy(batch.y_test.astype(
                np.int64)[:, :, 1]), requires_grad=False).cuda()
        ...
        return Episode(x_train,
                       ...
                       x_unlabel=x_unlabel,
                       y_unlabel=y_unlabel,
                       unlabel_indices=np.expand_dims(
                           batch.unlabel_indices, 0),
    ...
    def _noisify_labels(self, y_train, num_noisy=1):
        if num_noisy > 0:
            ...
            y_train[:, ::int(shot/num_noisy)] = np.random.randint(0,
                                                                  num_classes, len(selected_idxs[0]))
        return y_train

    def _run_forward(self, cnn_input):
        n_class = cnn_input.size(1)
        n_support = cnn_input.size(0)
        encoded = self.encoder.forward(cnn_input.view(
            n_class * n_support, *cnn_input.size()[2:]))
