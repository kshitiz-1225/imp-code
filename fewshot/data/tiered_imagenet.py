...
class TieredImageNetDataset(RefinementMetaDataset):
    ...
    def __init__(self,
        ...
        self._imagenet_train_folder = os.path.join(
            '/om/data/public/imagenet/', "images")
    ...
    def save_cache(self):
        ...
        if not os.path.exists(cache_path_labels):
            ...
            print("Saved cache in location {}".format(
                self.get_cache_path()[0]))
        # Save the images
        if not os.path.exists(cache_path_images):
            np.savez(self.get_cache_path()[1], images=self._images)
            print("Saved the images in location {}".format(
    ...
    def read_label_split(self):
        ...
        if os.path.exists(cache_path_labelsplit):
            self._label_split_idx = np.loadtxt(
                cache_path_labelsplit, dtype=np.int64)
        else:
            if self._split in ['train', 'trainval']:
            ...
            self._label_split_idx = np.array(
                self.label_split(), dtype=np.int64)
            self.save_label_split()

    def save_label_split(self):
        np.savetxt(self.get_label_split_path(),
                   self._label_split_idx, fmt='%d')
    ...
    def read_splits(self):
        ...
        with open(csv_path, 'r') as csvfile:
            csvreader = csv.reader(
                csvfile, delimiter=str(u','), quotechar=str(u'|'))
            ...
            for synset in tqdm(synset_dirs, desc="Reading dataset..."):
                ...
                for cat, synset_list in self._catcode_to_syncode.iteritems():
                    if synset in synset_list:
                        break
                synset_dir_path = os.path.join(
                    self._imagenet_train_folder, synset)
                ...
                for img_fname in img_list:
                    ...
                    if os.path.getsize(fpath) > 0:
                        ...
                        if img is not None:
                            img = cv2.resize(
                                img, (84, 84), interpolation=cv2.INTER_CUBIC)
