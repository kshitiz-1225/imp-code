...
class Episode(object):
    ...
    def next_batch_separate(self, classes, total_num_classes):
        example_range, y_train_labs = np.where(
            self._y_train[:, 1][:, None] == classes)
        ...
        test_idxs, y_test_labs = np.where(
            self._y_test[:, 1][:, None] == classes)
        ...
        if self._y_unlabel is not None and len(self._y_unlabel) > 0:
            unlabel_indices, unlabel_episode = np.where(self._y_unlabel[:, 1][:, None] == np.array(
                classes.tolist() + range(total_num_classes, total_num_classes + 1+len(classes))))
            x_unlabel = self._x_unlabel[unlabel_indices, :, :, :]
            y_unlabel = self._y_unlabel[unlabel_indices, :]
            unlabel_img_indices = (np.array(self._unlabel_indices)[
