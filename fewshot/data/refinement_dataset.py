...
class RefinementMetaDataset(object):
    ...
    def mode_split(self):
        ...
        for sup in range(0, len(self.coarse_labels)):
            mode_split.extend(list(np.random.choice(self.coarse_labels[sup], max(
                1, int(self._mode_ratio*len(self.coarse_labels[sup]))), replace=False)))
    ...
    def read_mode_split(self):
        ...
        if os.path.exists(cache_path_modesplit):
            self._class_train_set = np.loadtxt(
                cache_path_modesplit, dtype=np.int64)
        else:
    ...
    def next_episode(self, within_category=False):
        ...
        if self._mode_ratio < 1.0:
            ...
            self.class_seq = np.arange(num_label_cls)  # [0, 1, 2,  ... 4111]
        ...
        if within_category and self._cat_way != -1:
            ...
            cat_idxs = self._rnd.choice(cat_labels, min(
                self._cat_way, num_cats), replace=False)
            ...
            for cat_idx in cat_idxs:
                ...
                allowable_inds = np.concatenate(
                    (allowable_inds, filtered_inds[0:min(self._nway, len(filtered_inds))]))
            class_seq_i = (allowable_inds[1:]).astype(np.int64)
            self._rnd.shuffle(class_seq_i)
            total_way = len(class_seq_i)

        else:  # True
            total_way = self._nway  # 5
            class_seq_i = self.class_seq  # np.arrage(4112)

        is_training = self._split in ["train", "trainval"]
        assert is_training or self._split in ["val", "test"]

        for ii in range(total_way + self._num_distractor):  # range(5+0)

            cc = class_seq_i[ii]  # 4112
            _ids = self._label_idict[cc]  # 20

            # print(_ids)

            # Split the image IDs into labeled and unlabeled.
            _label_ids = list(
                filter(lambda _id: _id in self._label_split_idx_set, _ids))  # 8
            _unlabel_ids = list(
                filter(lambda _id: _id not in self._label_split_idx_set, _ids))  # 12
        ...
        if hasattr(self, "_category_labels"):
            train_labels_str = np.hstack((np.array(train_labels_str)[:, None], np.array(
                self.episodic_labels(train_labels_str))[:, None]))
            test_labels_str = np.hstack((np.array(test_labels_str)[:, None], np.array(
                self.episodic_labels(test_labels_str))[:, None]))
        else:
